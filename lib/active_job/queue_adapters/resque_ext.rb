module ActiveJob::QueueAdapters::ResqueExt
  ...
  def fetch_jobs(jobs_relation)
    unless jobs_relation.failed || jobs_relation.queue_name.present?
      raise ActiveJob::Errors::QueryError.new(jobs_relation), "This adapter only supports fetching failed jobs when no queue naem is provided"
    end

    resque_jobs = if jobs_relation.failed?
    ...
    resque_jobs.collect { |resque_job| deserialize_resque_job(resque_job) }
  end
