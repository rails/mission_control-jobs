module ActiveJob::QueueAdapters::SolidQueueExt
  ...
  def supported_filters(*)
    if jobs_relation.scheduled? then
      [ :queue_name, :job_class_name, :scheduled_at ]
    else
  ...
  class RelationAdapter
    ...
    def order_executions(executions)
      ...
      if solid_queue_status.scheduled? then
        executions.ordered
      else
    ...
    def matches_relation_filters?(job)
      matches_status?(job) && matches_queue_name?(job) && matches_scheduled_at_filter?(job)
    end
    ...
    def matches_scheduled_at_filter?(job)
      queue_name.blank? ||
        (job.scheduled_at.after.filter.scheduled_at_start &&
          job.scheduled_at.before.filter.scheduled_at_end)
    end
